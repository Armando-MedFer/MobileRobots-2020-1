...
#POSISION FINAL EN Y
    ...
    open_list      = deque()#CREAMOS UN ARREGLO
    in_open_list   = [False]*len(req.map.data) #ABRIMOS LA LISTA ABIERTA
    in_closed_list = [False]*len(req.map.data) #ABRIMOS LA LISTA CERRADA
    distances      = [sys.maxint]*len(req.map.data)#VARIABLE AUX DIST
    parent_nodes   = [-1]*len(req.map.data)
    
    current = start_idx#TOMAMOS EL NODO INNICIAL
    open_list.append(start_idx)#LO AGREGAMOS A LISTA ABIERTA
    in_open_list[start_idx] = True#REVISAMOS SI EXISTE EN OL
